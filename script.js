...
const validateCred = (arr) => {
  const tempArr = arr.map(x => x * 1)
for (i=tempArr.length - 2; i>=0 ; i-=2){
...
const findInvalidCards = (nestedArr) => {
  const tempArr = nestedArr
  let tempInvalidCards = [];  
  nestedArr.forEach((element, index) => {
    if (validateCred(element) === false){
      tempInvalidCards.push(element)    
   }})
return tempInvalidCards
}
...
return companies.sort().filter(function(item, pos, ary) {
